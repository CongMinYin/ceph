...
class ObjectCacheStore {
 ...
 private:
  class CacheObject {
    ...
    CacheObject(uint64_t pool_id, uint64_t snap_id,
                 uint64_t object_size, std::string object_name,
                 std::string pool_nspace)
      : pool_id(pool_id), snap_id(snap_id),
        object_size(object_size), object_name(object_name), pool_nspace(pool_nspace)
    {}
  };
  ...
  void handle_throttle_ready(void* ctx, uint64_t f);
  void apply_qos_tick_and_limit(const uint64_t flag, uint64_t min_tick,
                       uint64_t limit, uint64_t burst, uint64_t burst_seconds);
  ...
  static std::queue<CacheObject *> m_promote_queue;
};
